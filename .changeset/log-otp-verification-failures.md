...
- The same logging also covers the other OTP verification endpoints under `/email-otp/*` (e.g. `verify-email`, `reset-password`); the `path` field identifies which one. Only these OTP endpoints are logged.
